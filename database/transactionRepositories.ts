...
import { deleteEntry, editEntryAmount, insertEntry } from './entryRepositories'
...
export async function editEntryAmountAndItemStockToDb(newAmount: number, newEntryValue: number, entryId: string, newProductStock: number, productId: string): Promise<void> {

    try {

        await db.runAsync('BEGIN TRANSACTION')

        await editEntryAmount(newAmount, newEntryValue, entryId)
